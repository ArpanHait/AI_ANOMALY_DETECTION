...
export default function DashboardDark() {
  ...
  return (
    <div ref={containerRef} className="dashboard-dark-theme min-h-screen flex flex-col md:flex-row overflow-x-hidden font-body bg-[#0A0E17] text-on-surface">
      ...
      <main className="ml-0 md:ml-[15%] flex-grow flex flex-col md:flex-row h-auto min-h-screen md:h-screen pt-16 pb-24 md:pb-gutter px-gutter gap-panel-gap overflow-y-auto md:overflow-hidden w-full">
        {/* TopAppBar */}
        <header className="fixed top-0 left-0 md:left-[15%] right-0 h-16 z-40 bg-surface/10 backdrop-blur-md border-b border-outline-variant/5 flex items-center justify-between px-gutter transition-all duration-200 ease-in-out w-full md:w-[calc(100%-15%)]">
          <div className="flex items-center min-w-[120px] md:min-w-[240px]">
            <div className="font-headline-md text-sm md:text-headline-md font-bold text-primary-container">INSIGHT MONITOR</div>
          </div>
          <nav className="hidden md:flex flex-grow justify-center space-x-6 font-label-caps text-label-caps">
            <Link href="#" className="text-primary-container border-b-2 border-primary-container pb-1">Real-time</Link>
          ...
          <div className="flex items-center space-x-2 md:space-x-4 min-w-[120px] md:min-w-[240px] justify-end">
            <button className="bg-error-container/20 border border-error text-error font-label-caps text-[9px] md:text-label-caps px-2 md:px-4 py-1.5 md:py-2 rounded hover:bg-error-container/40 transition-colors uppercase whitespace-nowrap">
